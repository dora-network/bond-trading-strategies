...
type Config struct {
	...
	// CapturePendingSweepInterval is how often the janitor scans
	// strategy_capture_pending for stashes past CapturePendingRetention.
...
// (positive durations/limits, non-empty required fields). It does not
// re-check env-var parse errors — those are caught at read time in
