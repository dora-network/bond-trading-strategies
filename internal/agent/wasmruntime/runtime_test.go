...
func TestRuntime_Constructs(t *testing.T) {
	...
	rt, err := wasmruntime.NewRuntime(t.Context())
	if err != nil {
...
func TestRuntime_RequiresArtifactRoot(t *testing.T) {
	...
	if _, err := wasmruntime.NewRuntime(t.Context()); err == nil {
...
func TestRuntime_LoadPlugin(t *testing.T) {
	...
	st := rt.Store()
	wh, mh, err := st.Put([]byte("\x00asm\x01\x00\x00\x00"), []byte(`{"schema_version":1,"module_name":"x","framework_version":"`+prompts.WasmFrameworkVersion+`","language":"go","capabilities":{"order_books":["OB-1"],"host_functions":["host_log"]}}`))
	if err != nil {
