...
start-strategy-server:
	go run ./cmd/strategy-server -a "$(STRATEGY_ADDR)" -d "$(DATABASE_URL)" -w "$(WS_URL)" -k "$(DORA_API_KEY)" -b "$(DORA_BASE_URL)" -f "$(FRED_API_KEY)" -e "$(ENCRYPTION_KEY)" --cors-allowed-origins "https://dora-awsdev.vercel.app"
